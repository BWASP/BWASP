...
from Crawling.feature import get_cookies
from Crawling.feature import func
from Crawling.feature.db import (
    Packets as dbOfPackets,
    CSPEvaluator as dbOfCSPEvaluator,
    Domains as dbOfDomains,
    Ports as dbOfPorts,
    SystemInformation as dbOfSystemInformation
)
from Crawling.feature.api import Packets
...
def analysis(input_url, req_res_packets, cur_page_links, options, cookie_result, DETECT_LIST, LOCK, current_url, previous_packet_count, ANALYSIS_DATA):
    ...
    dbOfDomains().insertDomains(req_res_packets, cookie_result, packet_indexes, input_url, ANALYSIS_DATA)  # current_url을 input_url로 바꿈 openredirect 탐지를 위해 (11-07)_
    dbOfSystemInformation().updateWebInfo(DETECT_LIST[0])
...
def visit(driver, url, depth, options):
    ...
    if START_OPTIONS["check"]:
        ...
        dbOfSystemInformation().postWebInfo(driver.current_url)
        ...
        if "portScan" in options["tool"]["optionalJobs"]:
            ...
            dbOfPorts().insertPorts(target_port, START_OPTIONS["input_url"])
        else:
        ...
        if "CSPEvaluate" in options["tool"]["optionalJobs"]:
            ...
            dbOfCSPEvaluator().insertCSP(csp_result)
    ...
    dbOfPackets().insertPackets(packet_obj.packets)
