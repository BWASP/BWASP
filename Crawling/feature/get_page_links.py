...
class GetPageLinks:
    ...
    def start(self, url, html):
        self.page_source = BeautifulSoup(html, features="html.parser")
        cur_page_links = list()
        ...
        for tag_type in self.tag_type_list:
            for tag in self.page_source.find_all(tag_type): # tag_type 에 해당하는 tag 하나하나 접근
                if 'href' in tag.attrs: # tag에 href atrribute가 존재하는지 확인
                    cur_page_links.append(urljoin(url, tag.attrs['href']))
...
if __name__ == "__main__":
    ...
    for packet in req_res_packets:
        print(GetPageLinks().start(url, packet['response']['body']))
        input()